...
exports.prompts = exports.registerPrompts = void 0;
...
const registerPrompts = (server) => {
    ...
    server.prompt("check-wallet", checkWalletSchema.shape, ({ address, chain = "plasma" }) => ({
...
Aim to be concise but informative in your analysis.`,
    ...
    server.prompt("check-contract", checkContractSchema.shape, ({ address, chain = "plasma" }) => ({
...
Be analytical but accessible in your explanation.`,
    ...
    server.prompt("gas-analysis", gasAnalysisSchema.shape, ({ chain = "plasma" }) => ({
...
Keep your analysis concise, focusing on actionable insights.`,
    ...
    server.prompt("transaction-lookup", transactionLookupSchema.shape, ({ hash, chain = "plasma" }) => ({
...
Make the analysis clear and easy to understand.`,
    ...
    server.prompt("block-info", blockInfoSchema.shape, ({ chain = "plasma" }) => ({
...
const checkWalletSchema = zod_1.z.object({
    address: zod_1.z.string().refine(viem_1.isAddress, {
        message: "Invalid Ethereum address format",
    }),
    chain: zod_1.z.string().optional().default("plasma").refine((val) => Object.keys(chains_1.CHAINS).includes(val), {
        message: `Unsupported chain. Use one of: ${Object.keys(chains_1.CHAINS).join(", ")}`,
    }),
});
// Schema for check-contract prompt (with optional chain, defaults to plasma)
const checkContractSchema = zod_1.z.object({
    address: zod_1.z.string().refine(viem_1.isAddress, {
        message: "Invalid Ethereum address format",
    }),
    chain: zod_1.z.string().optional().default("plasma").refine((val) => Object.keys(chains_1.CHAINS).includes(val), {
...
const gasAnalysisSchema = zod_1.z.object({
    chain: zod_1.z.string().optional().default("plasma").refine((val) => Object.keys(chains_1.CHAINS).includes(val), {
        message: `Unsupported chain. Use one of: ${Object.keys(chains_1.CHAINS).join(", ")}`,
    }),
});
// Schema for transaction-lookup prompt (new)
const transactionLookupSchema = zod_1.z.object({
    hash: zod_1.z.string().regex(/^0x[a-fA-F0-9]{64}$/, {
...
const blockInfoSchema = zod_1.z.object({
    chain: zod_1.z.string().optional().default("plasma").refine((val) => Object.keys(chains_1.CHAINS).includes(val), {
        message: `Unsupported chain. Use one of: ${Object.keys(chains_1.CHAINS).join(", ")}`,
    }),
});
